...
const Dashboard = ({ user }) => {
  ...
  const fetchTransactions = async () => {
    setIsLoading(true);
    try {
      const res = await axios.get("https://localrepo-production-d703.up.railway.app/api/transactions", {
...
const handleEdit = (transaction) => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this transaction?")) {
      try {
        await axios.delete(`https://localrepo-production-d703.up.railway.app/api/transactions/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
      ...
      <div className="flex flex-col items-start justify-between gap-4 mb-6 md:flex-row md:items-center">
        ...
        <div className="flex p-1 space-x-1 bg-gray-100 rounded-lg dark:bg-gray-800">
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${activeTab === "all" ? 'bg-white dark:bg-gray-700 shadow-sm text-indigo-600 dark:text-purple-400' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'}`}
          >
            All
          </button>
          <button
            onClick={() => setActiveTab("income")}
            className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${activeTab === "income" ? 'bg-white dark:bg-gray-700 shadow-sm text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'}`}
          >
            Income
          </button>
          <button
            onClick={() => setActiveTab("expense")}
            className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${activeTab === "expense" ? 'bg-white dark:bg-gray-700 shadow-sm text-red-600 dark:text-red-400' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'}`}
          >
            Expenses
          </button>
        </div>
        ...
        <div className="flex space-x-3">
          ...
          <motion.button
            ...
            className={`flex items-center space-x-2 px-4 py-2 rounded-lg shadow-sm transition-all ${isFilterOpen ? 'bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white' : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'}`}
          >
            <FiFilter />
            <span>Filters</span>
          </motion.button>
          ...
          <motion.button
            ...
            className={`flex items-center space-x-2 px-4 py-2 rounded-lg shadow-sm transition-all ${isFormOpen ? 'bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white' : 'bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white'}`}
          >
            <FiPlus />
      ...
      >
        <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-gray-700">
          ...
          <div className="flex items-center space-x-2">
            ...
            {filters.type !== "all" || filters.category !== "all" || filters.dateRange !== "all" || filters.minAmount || filters.maxAmount ? (
